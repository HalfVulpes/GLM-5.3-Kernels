...
# Modified for per-token INT8 NoPE MLA and standalone PyTorch/Triton imports.
"""Titan SM80 NoPE MLA: per-token INT8 KV with fused dequantization.

Derived from vLLM 3bec275739c6f4cc7c2ff403d0556d477e4d0f33.
528 bytes/token: 512 signed INT8 values followed by 1 FP32 scale + 12 padding bytes.
"""
...
def num_compute_units(device_index):
...
# Measured on Titan at TP4 / 262144-token KV, see docs/BENCHMARKS.md.
_FINAL_AUTOTUNE_CONFIGS = [triton.Config({"BLOCK_N": 64}, num_warps=4, num_stages=1)]
...
def _sparse_mla_compute_tile(
    ...
    for start_indice in range(split_start, split_end, BLOCK_N):
        ...
        if k_buffer.dtype.element_ty == tl.int8:
            k = k.to(q.dtype)
            scale_ptr = (k_buffer + indices.to(tl.int64) * stride_kv_token + 512).to(tl.pointer_type(tl.float32))
            kv_scale = tl.load(scale_ptr, mask=mask_kv, other=0.0)
        ...
        if BLOCK_DPE == 0:
            # NoPE stores the same latent for K and V: reuse the dequantized tile.
            v = tl.trans(k).to(q.dtype)
        else:
            v = tl.load(k_buffer + offs_v, mask=mask_kv[:, None], other=0.0)

        n_e_max = tl.maximum(tl.max(qk, 1), e_max)
        re_scale = tl.exp2(e_max - n_e_max)
        p = tl.where(mask_kv[None, :] & mask_h[:, None], tl.exp2(qk - n_e_max[:, None]), 0.0)
        acc *= re_scale[:, None]
        if k_buffer.dtype.element_ty == tl.int8:
            acc += tl.dot((p * kv_scale[None, :]).to(v.dtype), v)
        else:
...
def triton_mla_sparse_attention(
    ...
    Args:
        ...
        kv:        [seq_kv, 1, 528] int8 packed cache for the tested NoPE path;
                   the inherited BF16 path uses [seq_kv, 1, dim_qk].
    ...
    assert kv.shape[1] == 1 and kv.shape[2] == (528 if kv.dtype == torch.int8 else dim_qk)
